...
impl IntDiffnSweep {
	...
	fn prune_min<P: PropagationActions>(actions: &mut P, o_idx: usize) -> Result<(), Conflict> {
		Ok(())
	}
	fn prune_max<P: PropagationActions>(actions: &mut P, o_idx: usize) -> Result<(), Conflict> {
		Ok(())
	}
	...
	fn generate_fr<P: PropagationActions>(
		...
		for i in 0..no_objects {
			...
			let is_overlapping = Self::overlaps::<P>(actions, &box_posn[o_idx], &fr, dimensions);
