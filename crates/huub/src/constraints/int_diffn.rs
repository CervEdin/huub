...
impl IntDiffnSweep {
	...
	fn prune_min<P: PropagationActions>(
		...
		// Start pruning here
		if b && sweep[curr_dimension]
			!= actions.get_int_lower_bound(self.box_posn[curr_obj_idx][curr_dimension])
		{
	...
	fn prune_max<P: PropagationActions>(
		...
		if b && sweep[curr_dimension]
			!= actions.get_int_upper_bound(self.box_posn[curr_obj_idx][curr_dimension])
		{
			let mut reason = Vec::new();
			for &o_idx in fr_support {
				for d in 0..self.dimensions {
