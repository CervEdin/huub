			...
			match c.id.deref() {
				...
				"huub_diffn_int" | "huub_diffn_nonstrict_int" => {
					...
					if let [x, y, dx, dy] = c.args.as_slice() {
						...
						let box_posn: Result<Vec<Vec<_>>, _> = vec![x, y]
							.iter()
							.map(|x| x.iter().map(|l| self.lit_int(l)).collect())
							.collect();

						let box_size: Result<Vec<Vec<_>>, _> = vec![dx, dy]
							.iter()
							.map(|x| x.iter().map(|l| self.lit_int(l)).collect())
							.collect();
						if is_nonstrict {
				...
				"huub_diffn_k_int" | "huub_diffn_nonstrict_k_int" => {
					...
					if let [box_posn, box_size] = c.args.as_slice() {
						let box_posn: Result<Vec<Vec<_>>, _> = self
							.arg_array(box_posn)
							.iter()
							.map(|x| x.iter().map(|l| self.lit_int(l)).collect())
							.collect();

						let box_size: Result<Vec<Vec<_>>, _> = self
							.arg_array(box_size)
							.iter()
							.map(|x| x.iter().map(|l| self.lit_int(l)).collect())
							.collect();
